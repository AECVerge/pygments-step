...
* `\\` — one reverse solidus, since a single one would open a control directive.
...
The `\X2\..\X0\` and `\X4\..\X0\` forms take one or more groups of four / eight
hexadecimal characters respectively, and `\X\` takes exactly two; the lexer
enforces both.
